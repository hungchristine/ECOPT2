...
#        if len(temp_attr.index)==3:
        temp_attr.loc[eq,tec][term] = default_fleet.__getattribute__(param).loc[eq,tec][term]*0.99
    ...
    for param in param_list:
        if exp_type=="simple":
            ...
            if param == 'enr_partab':
                ...
                tec = ['BEV']#,'ICE']
            for eq,tec,term in product(equation_list,tec,function_terms):
                run_id = f'{eq}_{tec}_{term}_term'
                run_id_list.append(run_id)
                log.info(f'Starting run {run_id}')
