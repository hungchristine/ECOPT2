...
run_name = r'Run_2019-09-26T13_19'
fp = r"C:\Users\chrishun\Box Sync\YSSP_temp\visualization output\Run_2019-09-26T13_19"
...
# 'fleet_vkm', 'veh_oper_dist',  'seg_batt_caps', 'A_terms_raw', 'prod_df', 'batt_list', 'temp_prod_df', 'temp_oper_df', 'temp_df', 'b_prod', 'temp_a', 'b_oper', 'A', 'B',
params = ['tec_add_gradient', 'occupancy_rate','passenger_demand','veh_stck_tot']
complex_params = ['B_prod', 'B_oper','enr_partab', 'veh_partab']
now = datetime.now().isoformat(timespec='minutes').replace(':','_')
gams_run = gams_runner.GAMSRunner()

def run_sensitivity_analysis(param, default_fleet,run_id):
...
def load_default():
    with open('run_def_baseline_def_def_def_def_iTEM2-Base2019-09-26T13_19.pkl','rb') as f:
        d = pickle.load(f)
        return d
