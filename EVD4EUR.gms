...
* function demand. Manufacturing and supply
...
ENR_TEC_CORRESPONDANCE(tec, enr)               feasible combinations of technology and energy (fuel)
...
$LOAD TEC_COMPONENT_CORRESPONDANCE
...
RECYCLED_COMPONENTS(newtec,fleetreg,year,age)   Total battery capacity sent to recycling per year          [kWh]
...
* MANUF_CNSTRNT input is in GWh; TEC_COMPONENT_CORRESPONDANCE is in kWh
EQ_MANUF_CONSTRAINT(newtec,optyear)..                   MANUF_CNSTRNT(newtec,optyear)*1e6 =g= sum((seg, fleetreg), STOCK_ADDED(newtec,seg,fleetreg,optyear,new) * TEC_COMPONENT_CORRESPONDANCE(newtec,seg))
...
* Calculate amount of batteries retired (and collected) each year in kWh
EQ_RECYCLED_COMPONENTS(newtec,fleetreg,optyear,age)..             RECYCLED_COMPONENTS(newtec,fleetreg,optyear,age) =e= sum((seg), STOCK_REMOVED(newtec,seg,fleetreg,optyear,age)* TEC_COMPONENT_CORRESPONDANCE(newtec,seg))*RECOVERY_PCT(newtec, optyear)
;

* Calculate amount of materials recovered from end-of-life batteries for recycling (i.e., to new batteries), in kg
EQ_RECYCLED_MAT(mat_cat,optyear)..                   RECYCLED_MAT(mat_cat,optyear) =e= sum((newtec,prodyear,age), sum((fleetreg), RECYCLED_COMPONENTS(newtec,fleetreg,optyear,age)) * COHORT_AGE_CORRESPONDANCE(optyear,prodyear,age) * MAT_CONTENT(newtec, mat_cat, optyear)) * RECYCLING_YIELD(mat_cat,optyear)
;
...
* MAT_MIX(mat_prod, model_year)*MAT_PROD_EL_INT(tec,seg,prodyear)*ENR_IMPACT_INT(imp,'elc','prod',prodyear)/1000
...
TOT_NEWTEC_COMP_RECYCLED(modelyear) = sum((seg), sum((newtec,fleetreg, age), STOCK_REMOVED.l(newtec,seg,fleetreg,modelyear,age)* TEC_COMPONENT_CORRESPONDANCE(newtec,seg)));
MAT_REQ_TOT(mat_cat,modelyear) = sum((newtec,seg, fleetreg), STOCK_ADDED.l(newtec,seg,fleetreg,modelyear,new)*TEC_COMPONENT_CORRESPONDANCE(newtec,seg)*MAT_CONTENT(newtec,mat_cat,modelyear));
...
OPER_IMPACT_COHORT(imp,tec, seg, fleetreg,modelyear,prodyear,age) = sum((enr), TOT_STOCK.l(tec,seg,fleetreg,modelyear,age) * COHORT_AGE_CORRESPONDANCE(modelyear,prodyear,age)* TEC_OPER_IMPACT_INT(imp,tec,enr,seg,fleetreg,modelyear,prodyear,age) * ANNUAL_USE_INTENSITY(fleetreg,modelyear));
...
TOTAL_OPERATION_ENERGY(tec,seg,fleetreg,modelyear) = sum((prodyear, age), TOT_STOCK.l(tec,seg,fleetreg,modelyear,age) * COHORT_AGE_CORRESPONDANCE(modelyear,prodyear,age) * TEC_OPER_EINT(tec,seg,prodyear) * ANNUAL_USE_INTENSITY(fleetreg,modelyear));
